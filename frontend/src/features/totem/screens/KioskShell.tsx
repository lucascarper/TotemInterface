import type { ReactNode } from "react";
import { Logo } from "@/components/Logo";
import { IconBack, IconWifiOff } from "@/components/Icons";

interface Props {
  children: ReactNode;
  online: boolean;
  showBack: boolean;
  onBack: () => void;
}

export function KioskShell({ children, online, showBack, onBack }: Props) {
  return (
    <div className="kiosk brand-arc flex min-h-[100dvh] flex-col">
      <header className="flex items-center justify-between px-8 pt-7 pb-2">
        <div className="w-28">
          {showBack && (
            <button onClick={onBack} className="btn-ghost -ml-3 text-lg" aria-label="Voltar">
              <IconBack className="h-7 w-7" /> Voltar
            </button>
          )}
        </div>
        <Logo className="h-14 md:h-16" />
        <div className="flex w-28 justify-end">
          {!online && (
            <span className="badge bg-brand-red-50 text-brand-red gap-1.5 py-1">
              <IconWifiOff className="h-4 w-4" /> Sem rede
            </span>
          )}
        </div>
      </header>
      <main className="flex flex-1 flex-col items-center justify-center px-6 pb-10">{children}</main>
      <footer className="pb-4 text-center text-xs font-medium tracking-[.18em] text-ink-soft">
        SAÚDE E SEGURANÇA DO TRABALHO
      </footer>
    </div>
  );
}
